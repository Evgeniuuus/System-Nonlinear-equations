...
def phi_1(x, y):
    return np.sign(x) * np.sqrt(4 - y ** 2)
...
y_up = np.sqrt(4 - x_1 ** 2)
y_down = -np.sqrt(4 - x_1 ** 2)
...
ax.set_xlim([-3.5, 3.5])
ax.set_ylim([-3, 3])
plt.plot(x_1, y_up, color="red", label="x^2 + y^2 - 4 = 0")
plt.plot(x_1, y_down, color="red")
plt.plot(x_2, y_2, '--', color="green", label="3*x^2 - y = 0")
...
print(f"Решение: x = {x_0:.6f}, y = {y_0:.6f}")
if not switch:
